...
const plantillacreate = async(req, res) => {
    ...
    if (plantilla.identificacion.tipoDte === "03") {
        ...
        try {
            const [createdPlantilla] = await db("invalidated")
                .returning("id")
                .insert(JsontoDB)
                .returning("*");
...
const getPlantillasByUserId = async(req, res) => {
    ...
    try {
        ...
        const plantilla = await db("invalidated").where({ id_emisor: usuarioid });
        if (!plantilla) {
...
const getbytypeandid = async(req, res) => {
    try {
        ...
        const plantillas = await db("invalidated")
...
const updatePlantilla = async(req, res) => {
    ...
    try {
        const plantilla = await db("invalidated")
            .where({ codigo_de_generacion: codigo_de_generacion })
            .first();
        if (!plantilla) {
            return res.status(404).json({ message: "plantilla no encontrado" });
        }
        await db("invalidated")
            .where({ codigo_de_generacion: codigo_de_generacion })
            .update(JsontoDB);
...
const getplantilla = async(req, res) => {
    ...
    try {
        //return all plantillas by user id
        const plantilla = await db("invalidated").where({ codigo_de_generacion: codigo_de_generacion });
...
const getPlantillasByUserIdAndName = async(req, res) => {
    ...
    try {
        ...
        const plantilla = await db("invalidated").where({ id_emisor: usuarioid }).where("re_name", "like", `%${namesearch}%`);
        if (!plantilla) {
            return res.status(404).json({ message: "plantilla no encontrado" });
        }
        console.log(plantilla);
...
const getPlantillasByUserIdAndDateRamge = async(req, res) => {
    ...
    try {
        //return all plantillas by user id
        const plantilla = await db("invalidated").where({ id_emisor: usuarioid }).whereBetween('fecha_y_hora_de_generacion', [start, end]);
        if (!plantilla) {
            return res.status(404).json({ message: "plantilla no encontrado" });
        }
        console.log(plantilla);
...
const getPlantillasByUserIdAndType = async(req, res) => {
    ...
    try {
        //return all plantillas by user id
        const plantilla = await db("invalidated").where({ id_emisor: usuarioid, tipo: type });
        if (!plantilla) {
            return res.status(404).json({ message: "plantilla no encontrado" });
        }
        console.log(plantilla);
